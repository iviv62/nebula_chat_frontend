...
export type WebRTCAdapterEvents = {
  ...
  onCallDuration?: (duration: number) => void;
};
...
export class WebRTCAdapter {
  ...
  async joinCall(): Promise<void> {
    ...
    try {
      ...
      if (typeof joinData.duration === "number") {
        this.events.onCallDuration?.(joinData.duration);
      }
  ...
  handleVoiceEvent(msg: Record<string, unknown>): void {
    ...
    if (typeof msg["duration"] === "number") {
      this.events.onCallDuration?.(msg["duration"]);
    }
